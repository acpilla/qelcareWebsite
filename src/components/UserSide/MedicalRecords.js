...
export default function MedicalRecords({ appointment, latestVital, onCreated }) {
 ...
 const [showForm, setShowForm] = useState(Boolean(appointment));
 ...
 setError("");
 try {
 const recordEndpoint = isPatient ? "/medical-records/me" : "/medical-records?limit=100";
 const [recordRes, appointmentRes] = await Promise.all([
 authFetch(recordEndpoint),
 canCreate ? authFetch("/appointments?limit=100") : Promise.resolve(null),
 ]);
 ...
 appointment_id: appointment.id || appointment.appointment_id || "",
 vital_id: latestVital?.id || latestVital?.vital_id || "",
 chief_complaint: appointment.chief_complaint || latestVital?.chief_complaint || current.chief_complaint,
 }));
 }, [appointment, latestVital]);
 ...
 setShowForm(false);
 await load();
 if (onCreated) onCreated(payload.record || payload.data);
 } catch (err) {
 setError(err.message);
 } finally {
 setSaving(false);
 ...
 <div>
 <div style={{ fontWeight: 900, color: "#162235" }}>Doctor Record Entry</div>
 <div style={{ color: "#6b778c", fontSize: 13 }}>Create consultation notes, diagnosis, treatment, and prescriptions.</div>
 </div>
 ...
 <textarea style={{...inputStyle, minHeight: 72 }} value={form.doctor_notes} onChange={(e) => updateField("doctor_notes", e.target.value)} />
 </Field>
 ...
 <ActionButton disabled={saving}>{saving ? "Saving..." : "Create Record"}</ActionButton>
 <ActionButton tone="secondary" onClick={() => setShowForm(false)}>Cancel</ActionButton>
 </div>
 </form>
 )}
 </Panel>
 ...
 <EmptyState title="No records found" detail={isPatient ? "Completed doctor records will show here." : "Create a record after consultation."} />
 ) : (
 ...
 {record.doctor_notes && <RecordBlock label="Doctor notes" value={record.doctor_notes} />}
 </article>
 ))}
 </div>
 )}
