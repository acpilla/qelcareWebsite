...
class User {
  ...
  static async createUser({
    ...
    const result = await pool.query(
      `INSERT INTO users
        (username, email, password, first_name, last_name, role_id, phone, gender, status)
  ...
  static async updateAdminDetails(userId, { phone = null, gender = null }) {
    const result = await pool.query(
      `UPDATE users
       SET phone = $1,
           gender = $2,
           updated_at = NOW()
       WHERE user_id = $3
       RETURNING user_id, phone, gender, updated_at`,
      [phone || null, gender || null, userId]
    );
    return result.rows[0] || null;
  }
}
